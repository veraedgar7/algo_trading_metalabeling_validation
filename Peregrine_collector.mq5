...
input int    ADX_Threshold      = 18;
input double Vol_Mult_Factor    = 1;     // Relative volume filter, 20% above average

int bbHandle, atrHandle, maH4Handle, adxHandle, maHandle, rsiHandle, adxHandle_macro;
...
bool TrendStrengthADX() {
   ...
   // Copy the current ADX value (main line, index 0)
   if(CopyBuffer(adxHandle, 0, 1, 1, adxBuffer) < 1) return false;
   // Institutional standard:
   // ADX < 20: no trend
   // ADX > 20 or 25: strong trend
...
void OnTick() {
   ...
   // Only trade if nothing is currently open
   if(PositionSelect(_Symbol) && PositionGetInteger(POSITION_MAGIC) == Magic_Number) return;
   ...
   if(CopyBuffer(bbHandle, 1, 0, 1, upper) <= 0 ||
      ...
      CopyBuffer(atrHandle, 0, 0, 1, atr) <= 0) return;
   ...
   if((Buy || Sell)) {
      ...
      last_opened_ticket = trade.ResultOrder(); // Store the position ticket
      last_trade_time = TimeCurrent();
      SaveAIData(-1, bbn, current_adx, current_rsi, vol_rel, h_sin, h_cos, dist, candle_stretch, last_opened_ticket, 0.0);
   }
}
...
void SaveAIData(int target, double bbn, double adx, double rsi, double v_rel, double h_sin, double h_cos, double dist, double stch, ulong ticket, double profit_real) {
   static bool first_trade = true;
   string filename = "Peregrine_Data_Log.csv";
   int file_handle = INVALID_HANDLE;

   if(first_trade) {
      file_handle = FileOpen(filename, FILE_WRITE|FILE_CSV|FILE_ANSI|FILE_COMMON, ',');
      first_trade = false;
   }
   else {
      file_handle = FileOpen(filename, FILE_READ|FILE_WRITE|FILE_CSV|FILE_ANSI|FILE_COMMON, ',');
   }
   ...
   if(file_handle != INVALID_HANDLE) {
      ...
      if(FileSize(file_handle) == 0) {
         FileWrite(file_handle, "target", "bb_norm", "adx", "rsi", "vol_rel", "hour_sin", "hour_cos", "dist_MA", "stch", "ticket", "profit_real");
      }

      // Write the trade data
      FileWrite(file_handle, target, bbn, adx, rsi, v_rel, h_sin, h_cos, dist, stch, (string)ticket, profit_real);
      ...
      if(HistoryDealSelect(trans.deal)) {
         ...
         if(entry_type == DEAL_ENTRY_OUT) {
            if(HistoryDealGetString(trans.deal, DEAL_SYMBOL) == _Symbol) {
               ...
               // Target 1 if the net result is positive, 0 if it is a loss
               // Sum ALL deals for that position (opening, partial closes, commissions, swaps)
               for(int i=0; i<total_deals; i++) {
                  ulong d_ticket = HistoryDealGetTicket(i);
                  net_total += HistoryDealGetDouble(d_ticket, DEAL_PROFIT);
                  net_total += HistoryDealGetDouble(d_ticket, DEAL_COMMISSION);
                  net_total += HistoryDealGetDouble(d_ticket, DEAL_SWAP);
               }

               int target = (net_total > 0) ? 1 : 0;
               // Update the CSV record, now including the real profit for that position
               SaveAIData(target, 0, 0, 0, 0, 0, 0, 0, 0, position_id, net_total);
               last_trade_time = TimeCurrent();
            }
         }
      }
